...
group
	...
	.route("POST", "/goapi/getSysTemplateAttributes/", (req, res) => {
		const id = req.body?.asset.id || req.body?.asset.starter_id;
		res.assert(id, 400, { status: "error" });
	...
	.route("POST", "/goapi/getSysTemplates/", async (req, res) => {
		...
		if (req.body.type == "movie") {
			...
			const tXml = `<theme id="business" name="Business Friendly" cc_theme_id="business"><starter id="0pHH-WrskRNI" numScene="1" title="Text - Default" thumbnail="/stock_thumbs/278841979.jpg" enc_asset_id="0tkTvtxum130iuQ0JDOFAuw" itemRef="/alvin-dev/asset/63122">
		<film copyable="0" duration="300.0" published="0" pshare="0" isWide="1">
			<meta>
				...
				<desc />
				<mver><![CDATA[4]]></mver>
				<studio>d6aa0e223ee416886418814d10813bf3c7b5f50f</studio>
				<thumbnail index="0" />
				<palette>
					<color value="333333" />
					<color value="999999" />
			...
			<scene id="SCENE1" adelay="102" lock="N" index="0" color="8370961" guid="732FE15F-7374-96DF-E5B2-C5D62673D3BB">
				<durationSetting countMinimum="1" countTransition="1" countAction="1" countBubble="1" countSpeech="1" />
				<bg id="BG1851" index="0">
					<file>business.g4b_mbg041.swf</file>
					<color r="Color_A">2672321</color>
				</bg>
				<char id="AVATOR1849" index="1" raceCode="1">
					<action face="1" motionface="1">ugc.120491861.hand_up_indicate_2.xml</action>
					<x>192.2</x>
					<y>342.6</y>
					<xscale>2.84</xscale>
					<yscale>2.84</yscale>
				...
				<bubbleAsset id="BUBBLE10840" index="2" componentRole="title">
					<x>410.6</x>
					<y>191.6</y>
					<xscale>1</xscale>
					<yscale>1</yscale>
					...
					<fx>easeNone</fx>
					<fxdur>0</fxdur>
					<bubble x="-131" y="-26.5" w="282" h="52.9" rotate="0" type="BLANK" hasTail="0" colorStyle="1" autoTextResize="true">
						<body rgb="3621967" shadow="1" bgAlpha="0" linergb="0" tailx="90" taily="65" paddingHorizontal="20" paddingVertical="0" />
						<text rgb="16514043" alpha="100" font="LeagueGothic" size="40" align="right" bold="false" italic="false" embed="true"><![CDATA[Your title here]]></text>
						<url target="top" />
					</bubble>
				</bubbleAsset>
				<bubbleAsset id="BUBBLE10841" index="3" componentRole="subtitle">
					<x>406.5</x>
					<y>218.1</y>
					<xscale>1</xscale>
					<yscale>1</yscale>
					...
					<fx>easeNone</fx>
					<fxdur>0</fxdur>
					<bubble x="-135.1" y="-12.6" w="270.2" h="25.2" rotate="0" type="BLANK" hasTail="0" colorStyle="1" autoTextResize="true">
						<body rgb="16514043" shadow="1" bgAlpha="0" linergb="0" tailx="90" taily="65" paddingHorizontal="0" paddingVertical="0" />
						<text rgb="3621967" alpha="100" font="Lato" size="17" align="right" bold="false" italic="false" embed="true"><![CDATA[Your subtitle here]]></text>
						<url target="top" />
					</bubble>
				</bubbleAsset>
				<effectAsset id="EFFECT4" themecode="common" index="4">
				...
				<trans id="null" index="5">
					<fx dur="24" type="fl.Fade.out" ease="null" />
				</trans>
				<aTranList>
					<aTran id="752" target="BUBBLE10840" type="Slide" atype="BubbleAsset" direction="0" section="5" timing="0" duration="18" delay="0">
						<aTranSetting dest="6" />
					</aTran>
					<aTran id="753" target="BUBBLE10841" type="Slide" atype="BubbleAsset" direction="0" section="5" timing="3" duration="24" delay="0">
						<aTranSetting dest="6" />
					</aTran>
				</aTranList>
			</scene>
	...
	.route("POST", "/goapi/getSysTemplateAttributes/", async (req, res) => {
		const meta = DB.select("comm", "movie", "goapi", "getSysTemplateAttributes", "getSysTemplateAttributes", "starterTemplates");
		const zip = nodezip.create();
		let isProp;
		if (req.body.type == "movie", "goapi", "getSysTemplateAttributes", "getSysTemplateAttributes", "starterTemplates") {
			isMovie = true;
		}
		let xmltext = isProp ? "moreProp" : "moreBG";
		console.log(xmltext);
		...
		if (req.body.type == "movie") {
		...
		else if (req.body.type == "movie", "goapi", "getSysTemplateAttributes", "getSysTemplateAttributes", "starterTemplates") {
			const myname = path.join(sFolder, "CommFolder");
			const tXml = ``;
			const zip = nodezip.create();
			fUtil.addToZip(zip, "desc.xml", Buffer.from(tXml));
			fs.readdirSync(myname).forEach(CommFolder => {
				fs.readdirSync(path.join(myname, CommFolder)).forEach(file => {
					if (file.includes("movie", "goapi", "getSysTemplateAttributes", "getSysTemplateAttributes", "starterTemplates")) {
						fs.readdirSync(path.join(myname, `${CommFolder}/goapi/getSysTemplateAttributes/getSysTemplateAttributes/starterTemplates`)).forEach(file2 => {
							const buffer = fs.readFileSync(path.join(myname, `${CommFolder}/goapi/getSysTemplateAttributes/getSysTemplateAttributes/starterTemplates/${file2}`));
							fUtil.addToZip(zip, `Comm/${CommFolder}/goapi/getSysTemplateAttributes/getSysTemplateAttributes/starterTemplates/${file2}`, buffer);
						})
					} else {
						console.log(`${CommFolder}/${file}`);
						const buffer = fs.readFileSync(path.join(myname, `${CommFolder}/${file}`));
	...
	.route("POST", "/goapi/updateSysTemplateAttributes/", async (req, res) => {
		const meta = DB.select("comm", "movie", "goapi", "getSysTemplates", "getSysTemplates", "starterTemplates");
		const zip = nodezip.create();
		let isProp;
		if (req.body.type == "movie", "goapi", "getSysTemplates", "getSysTemplates", "starterTemplates") {
	...
	.route("POST", "/ajax/saveUserProp/", async (req, res) => {
		const file = req.files.import;
		res.assert(
			file,
			req.body.type,
	...
	// char
	.route("GET", /\/char_thumbs\/([\S]+)/, (req, res) => {
		const filepath = path.join(__dirname, "../../", thumbUrl, req.matches[1]);
		if (fs.existsSync(filepath)) {
			fs.createReadStream(filepath).pipe(res);
		} else {
